...
class ProfiledWanInferencePipeline(nn.Module):
    def __init__(self, args):
        ...
        with TimingContext("Model Loading"):
        ...
        if args.use_audio:
            with TimingContext("Audio Encoder Loading"):
    ...
    def load_model(self):
        ...
        # Load models
        with TimingContext("Model Manager Loading"):
        ...
        with TimingContext("Pipeline Creation"):
        ...
        with TimingContext("Model State Loading"):
            if args.train_architecture == "lora":
    ...
    def forward(self, prompt, 
        ...
        print(f"\n🚀 Starting inference for: '{prompt[:50]}...'")
        print(f"📊 Config: {num_steps or self.args.num_steps} steps, {seq_len} frames, TeaCache: {self.args.tea_cache_l1_thresh}")
        ...
        with TimingContext("Image Processing"):
        ...
        print(f"📐 Video dimensions: {select_size}, {L} frames, {T} latent frames")
        ...
        with TimingContext("Audio Processing"):
        ...
        if args.i2v:
            with TimingContext("Initial VAE Encoding"):
        ...
        for t in range(times):
            ...
            print(f"\n📹 Processing chunk [{t+1}/{times}]")
            
            with TimingContext(f"Chunk {t+1} Setup"):
                audio_emb = {}
                if t == 0:
                ...
                if audio_embeddings is not None:
                    ...
                    audio_tensor = audio_tensor.unsqueeze(0).to(self.device, dtype=self.dtype)
            ...
            with TimingContext(f"Chunk {t+1} Diffusion Process ({num_steps} steps)"):
            ...
            print(f"✅ Chunk {t+1} completed in {chunk_time:.3f}s")
            
        with TimingContext("Final Video Assembly"):
            video = torch.cat(video, dim=1)
            video = video[:, :ori_audio_len + 1]
            
        total_time = time.time() - total_start
        video_duration = video.shape[1] / self.args.fps
        fps_generation = video_duration / total_time
        
        print(f"\n🎯 PERFORMANCE SUMMARY:")
        print(f"   Total inference time: {total_time:.3f}s")
        print(f"   Video duration: {video_duration:.3f}s")
        print(f"   Generation speed: {fps_generation:.3f}x real-time")
        print(f"   Diffusion time: {total_diffusion_time:.3f}s ({total_diffusion_time/total_time*100:.1f}%)")
        print(f"   Steps per second: {num_steps * times / total_diffusion_time:.1f}")
...
def main():
    ...
    print("🔧 PROFILED INFERENCE STARTING")
    print(f"📋 Configuration: {args.config}")
    print(f"🎯 Target: {seq_len} frames, {args.num_steps} steps")
    
    inferpipe = ProfiledWanInferencePipeline(args)
    if args.sp_size > 1:
        date_name = inferpipe.pipe.sp_group.broadcast_object_list([date_name])
        date_name = date_name[0]
        
    output_dir = f'demo_out/{exp_name}/profiled_res_{os.path.splitext(os.path.basename(args.input_file))[0]}_'\
